...
//    TokenUtils.printTheTokens();
    ...
    // the current security context (user, roles,...) is BOUND TO THE (HTTP) THREAD
    dto.username = SecurityContextHolder.getContext().getAuthentication().getName();
    // proof: this fails:
//    dto.username = CompletableFuture.supplyAsync(() -> SecurityContextHolder.getContext().getAuthentication().getName())
//            .get();
